...
**[DB 로드]** 탭에서 스프레드시트 URL · enum 시트명(콤마 구분, 기본 `_Enum`) · enum 출력 파일명(기본 `GameEnum.cs`) 입력 후 **구글 시트 DB 최신화** 클릭.

한 번 누르면 다음이 모두 자동으로 처리된다.
- 데이터 시트 → `Assets/Resources/GoogleSheetData/*.json`
- enum 시트 → `Assets/Scripts/GeneratedEnums/<출력파일>.cs` 한 파일에 합쳐 출력
- 데이터 시트마다 → `Assets/Scripts/Data/Generated/`에 partial 데이터 클래스 + abstract 컨테이너 자동 생성
- 시트에서 사라진 테이블의 JSON · 자동 생성 코드는 같이 정리됨 (`PartialClass/`의 사용자 코드는 안 건드림)

이후 시트가 변경될 때마다 **"최신화" 버튼만 누르면** 끝이다.
...
- 컨테이너는 `Dictionary` / `List` 두 종류 기본 제공 (`DictionaryContainer`, `ListContainer`).
...
- 로드 흐름은 `Load → Validate → AfterAllTableLoaded` 3단계.
  `Validate`로 데이터 무결성 체크, `AfterAllTableLoaded`로 컨테이너 간 참조 캐싱.
- `DictionaryContainer`는 id 중복 시 `"<DB명> DB의 id 'X'가 중복되었습니다"` 에러 로그 후 첫 항목 유지.
- `GetEqualityComparer()` 훅으로 enum/string 키 박싱 회피·대소문자 무시 비교 등 가능.
...
DataContainerCodeGenerator ──►  Assets/Scripts/Data/Generated/*.cs
EnumCodeGenerator          ──►  Assets/Scripts/GeneratedEnums/<출력파일>.cs (단일)
   │
   ▼
DataManager (런타임)
   ...
   └─ DataContainer<TKey, TValue> 자동 등록 + 로드        (타입 세이프)
         ...
         └─ Load → Validate → AfterAllTableLoaded 순으로 호출
```
...
| `SheetJsonConverter` | CSV → `SheetData` JSON 변환, `#` 접두사 시트/컬럼 제외, sync 시 stale 파일 정리 |
| `DataContainerCodeGenerator` | 시트 → `Generated/<X>.cs` + `Generated/Containers/<X>DictionaryContainer.cs` 자동 생성 |
| `EnumCodeGenerator` | 지정된 enum 시트 → 단일 `.cs` (기본 `GameEnum.cs`)로 합쳐 생성 |
...
| `DataContainer<TKey, TValue>` | 시트 1장을 묶어두는 베이스. `Parse / Validate / AfterAllTableLoaded` 훅 제공 |
...
- enum 시트는 윈도우의 'Enum 시트명' 입력으로 지정 (콤마 구분, 기본 `_Enum`)
  - 컬럼 단위로 enum 정의 — Row 1: enum 타입명 / Row 2+: enum 멤버
  - 지정된 시트들의 모든 enum이 한 파일로 합쳐짐 (기본 `GameEnum.cs`)
...
> 시트가 사라진 테이블의 JSON · 자동 생성 코드는 sync 시 자동 삭제 (`// <auto-generated />` 헤더가 있는 파일만 대상이므로 사람이 만든 파일은 안전).
